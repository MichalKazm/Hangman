...
mainLoop() {
	...
	do
		...
		if [ "$LIVES" -le 0 ]; then
			...
			printf "%s" "$(date)" >> ./history/gameHistory
			echo ";Lost;$WORD;$FINAL_WORD;$GUESSED_LETTERS" >> ./history/gameHistory
		...
		if isGuessed; then
			...
			# Saving game to history
			printf "%s" "$(date)" >> ./history/gameHistory
                        echo ";Won;$WORD;$GUESSED_LETTERS;$LOST_LIVES;$SCORE" >> ./history/gameHistory
...
# Start menu
CHOICE=$(zenity --list --height=340 --width=200 --title "Menu" --text "Choose an option" --column "Options:" "New game" "Ranking" "History" "Settings")
case "$CHOICE" in
	"New game")
		mainLoop
	;;
